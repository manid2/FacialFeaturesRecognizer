...
ErrorCode BaseRecognizer::loadClassifier(const FFR::String& /*fileName*/) {
  ...
  do {  // for common error handling
    ...
    cv::String haarPath;
    haarPath = cv::format("%s/haarcascade_frontalface_default.xml", getenv(FFR_DATA_PATH));
    m_faceCascade.load(haarPath);
  ...
  do {  // for common error handling
    ...
    for (size_t fa = 0; fa < faces.size(); fa++) {
      ...
      for (; itr_ffr != m_ffrVec.end(); itr_ffr++) {
        FFR::String res = (*itr_ffr)->getResult(mlMat);
...
ErrorCode BaseRecognizer::computeHOG(const cv::Mat& img, cv::Mat& hogMat) {
  ErrorCode err = FFR::OK;
  vector<float> descriptors;
  do {  // for common error handling
    m_HOGDescriptor.winSize = img.size() / 8 * 8;
    m_HOGDescriptor.compute(img, descriptors);
    hogMat = Mat(descriptors).clone();
  } while (0);
  return err;
}

ErrorCode BaseRecognizer::convertToML(const cv::Mat& hogMat, cv::Mat& mlMat) {
  ErrorCode err = FFR::OK;
  do {  // for common error handling
    if (hogMat.empty()) {
      err = FFR::MlError;

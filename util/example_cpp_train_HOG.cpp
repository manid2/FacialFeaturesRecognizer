...
CTrainTestHOG::CTrainTestHOG()
  ...
  m_features.insert(FFR_UTIL::Age);
  m_features.insert(FFR_UTIL::Emotion);
  m_features.insert(FFR_UTIL::Gender);
  ...
  do {  // for common error handling
    ...
    for (size_t fa = 0; fa < faces.size(); fa++) {
      ...
      for (size_t i = 0; i < m_features.size(); ++i, ++iter_ft) {
        ...
        ResultPair res_pair((FFR_UTIL::Feature)i, res_s);
  ...
  do {  // for common error handling
    for (size_t fa = 0; fa < faces.size(); fa++) {
      ...
      for (auto res_pair : results.at(fa)) {
        ...
        const FFR_UTIL::Feature& f = res_pair.first;
...
int main(int argc, char **argv)
{
  ...
  DEBUGLF("enter\n");
  int ret = 0;
  ret = FFR_UTIL::execute(argc, argv);
  DEBUGLF("exit\n");
  return ret;
}
